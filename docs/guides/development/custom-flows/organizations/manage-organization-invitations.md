...
> Learn how to use the Clerk API to build a custom flow for creating and managing Organization invitations.
...
> [!NOTE]
> This guide is for creating and managing Organization invitations client-side. You can also create an Organization invitation using the Backend API. See the [Organization invitations reference](/guides/organizations/add-members/invitations) for more information.
>
> Also, see the [custom flow for accepting Organization invitations](/guides/development/custom-flows/organizations/accept-organization-invitations).
...
> Examples for this SDK aren't available yet. For now, try adapting the  available example to fit your SDK.
  ...
  1. Use the [`useOrganization()`](/reference/hooks/use-organization) hook to get `organization`, which is the Active Organization's.
  1. Use `organization` to call the [`inviteMember()`](/reference/javascript/organization#invite-member) method, with the recipient's email address and desired Role passed as arguments.
  ...
  1. `invitations` is an array of [`OrganizationInvitation`](/reference/javascript/types/organization-invitation) objects. Each `OrganizationInvitation` object has a [`revoke()`](/reference/javascript/types/organization-invitation#revoke) method that can be called to revoke the invitation.

  The following example includes:

  - An `` component that allows administrators to invite new members to their Organization.
  - An `` component that lists all pending invitations and allows administrators to revoke them.

  
    ```tsx
// Filename: app/components/InvitationList.tsx
    ...
    ```tsx
// Filename: app/components/manage-organization-invitations.tsx
    ...
    const SelectRole = (props: SelectRoleProps) => {
      ...
      const { organization } = useOrganization()
      const [fetchedRoles, setRoles] = React.useState([])
  ...
  1. Call the [`getOrganizationMemberships()`](/reference/javascript/user#get-organization-memberships) method to get a list of Organizations that the user is a member of. This method returns `data`, which is an array of `OrganizationMembership` objects.
  ...
  1. Use the active `organization` object to call the [`inviteMember()`](/reference/javascript/organization#invite-member) method, with the recipient's email address and desired Role passed as arguments.

  To revoke an invitation:

  1. Use the active `organization` object to call the [`getInvitations()`](/reference/javascript/organization#get-invitations) method to get an array of `OrganizationInvitation` objects.
  1. Each `OrganizationInvitation` object has a [`revoke()`](/reference/javascript/types/organization-invitation#revoke) method that can be called to revoke the invitation.
...
// Filename: main.js
  ...
  ```swift
// Filename: ManageOrganizationInvitationsView.swift
  ...
  suspend fun fetchPendingInvitations(): List {
...
Now that you've created a flow for managing Organization invitations, you might want to create a flow for accepting invitations. See the [dedicated custom flow guide](/guides/development/custom-flows/organizations/accept-organization-invitations) for more information.
