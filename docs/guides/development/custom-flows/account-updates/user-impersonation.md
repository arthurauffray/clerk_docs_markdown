...
> Learn how to build a custom flow using the Clerk API that handles user impersonation.
...
[Clerk's user impersonation feature](/guides/users/impersonation) allows you to sign in to your application as one of your users, enabling you to directly reproduce and remedy any issues they're experiencing. It's a helpful feature for customer support and debugging.
...
> Examples for this SDK aren't available yet. For now, try adapting the  available example to fit your SDK.
  ...
  - The main page that gets the list of the application's users using the [JS Backend SDK](/reference/backend/user/get-user-list)
  - The Client Component that has the UI for displaying the users and the ability to impersonate them
  - The Server Action that generates the actor token using the [Backend API](/reference/backend-api/tag/actor-tokens/post/actor_tokens)
...
```tsx
// Filename: app/dashboard/_components.tsx
...
```tsx
// Filename: app/dashboard/_actions.ts
...
// Filename: app/dashboard/_layout.tsx
    ...
    Create the `generateActorToken+api.tsx` file with the following code. This creates an API route that will call Clerk's Backend API [`/actor_tokens`](/reference/backend-api/tag/actor-tokens/post/actor_tokens) endpoint to create an actor token.
