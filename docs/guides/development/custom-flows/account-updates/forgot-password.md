...
> This guide is for users who want to build a custom flow. To use a _prebuilt_ UI, use the [Account Portal pages](/guides/account-portal/overview) or [prebuilt components](/reference/components/overview).
  ...
  >
  > If you're using an older version of one of these SDKs, or are using the legacy API, refer to the [legacy API documentation](/guides/development/custom-flows/account-updates/legacy/forgot-password).
...
// Filename: app/components/forgot-password.tsx
  ...
  ```swift
// Filename: ForgotPasswordView.swift
...
```kotlin
// Filename: ForgotPasswordEmailViewModel.kt
...
```kotlin
// Filename: ForgotPasswordEmailActivity.kt
...
```kotlin
// Filename: ForgotPasswordPhoneViewModel.kt
...
```kotlin
// Filename: ForgotPasswordPhoneActivity.kt
...
If you have enabled [rejection of compromised passwords also on sign-in](/guides/secure/password-protection-and-rules#reject-compromised-passwords), then it is possible for the sign-in attempt to be rejected with the `form_password_pwned` error code.
