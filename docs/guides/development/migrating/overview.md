...
> Guides on how to move your user data from another authentication platform to Clerk.

## Migration tooling

To aid in basic migrations, Clerk provides an open-source tool that takes a JSON or CSV file as input, containing a list of users, and creates a user in Clerk using the Backend API. The CSV can be a custom file or an [export from the Clerk Dashboard](#export-your-users-data-from-the-clerk-dashboard). The tool respects the [backend rate limits](/guides/how-clerk-works/system-limits#backend-api-requests) and gracefully handles errors. We suggest you customize the [Zod schema](https://github.com/clerk/migration-tool/blob/main/src/migrate/validator.ts) to your application's needs.
...
With basic export/import, you're taking an export from your previous tool and importing data into Clerk. The most common way to handle this is by making use of the [`CreateUser`](/reference/backend-api/tag/users/post/users) Backend API endpoint. It's important to note that the `CreateUser` endpoint is rate limited. For more information, see the [guide on rate limits](/guides/how-clerk-works/system-limits#backend-api-requests).

You'll also need to provide your `password_hasher` value (the hashing algorithm used to generate the password digest) and in some instances Clerk will transparently upgrade your users' password hashes to the more secure Bcrypt hashing algorithm. More details on this topic are available in the [Backend API reference docs](/reference/backend-api/tag/Users#operation/CreateUser!path=password_hasher\&t=request).
...
> [!NOTE]
> It's important to note that Clerk only charges by Monthly Retained Users (MRUs) and never based on your total number of users in the user table.
>
...
You can programmatically retrieve a list of your user's and their information by using the [`GetUserList`](/reference/backend-api/tag/users/get/users) Backend API endpoint, or the JS Backend SDK's [`getUserList()`](/reference/backend/user/get-user-list) method which is a wrapper around the Backend API endpoint.
