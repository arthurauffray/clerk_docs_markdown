...
# Filename: terminal
  ...
  firebase auth:export firebase-users.json --format=json --project 
  ```
  ...
  1. `--project `: it tells Firebase which project you're trying to export from. Don't forget to replace `` with the Project ID you found in the previous step.
...
# Filename: terminal
  ...
  > [!WARNING]
  > This implementation does not take rate limiting into account. If you have a large user base, you may want to implement a retry mechanism. Clerk will return a `429` status code if you exceed the rate limit. You can find more information about Clerk's rate limits [here](/guides/how-clerk-works/system-limits). Keep [Firebase's rate limits](https://firebase.google.com/docs/functions/quotas) in mind as well when exporting your users.
