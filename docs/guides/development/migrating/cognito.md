...
the [Backend API](/reference/backend-api/tag/users/post/users).

- `password_hasher`: `awscognito`
- `password_digest`: `awscognito###<identifier>`
...
> [!CAUTION]
> [Rate limits](/guides/how-clerk-works/system-limits) apply to the [`CreateUser`](/reference/backend-api/tag/users/post/users) endpoint. For development instances, a rate limit rule of **100 requests per 10 seconds** is applied. For production instances, that limit goes up to **1000 requests per 10 seconds**.
...
you will be able to validate the migration behavior.
  ...
  update your application to use Clerk’s managed [Account Portal](/guides/account-portal/overview) or the [``](/reference/components/authentication/sign-in) component.
