...
> Learn how to use the useLocalCredentials() hook in your Expo app to enable biometric sign-ins for returning users.

> [!WARNING]
> This feature requires `@clerk/expo` or `@clerk/clerk-expo >=@2.2.0` and works only for sign-in attempts that use the password strategy.

This guide demonstrates how to use the [`useLocalCredentials()`](/reference/expo/native-hooks/use-local-credentials) hook in your Expo app to securely store a user's password credentials on their device, enabling biometric sign-in for returning users.
...
// Filename: app/sign-in.tsx
  ...
  ## Delete credentials while user is signed in
...
// Filename: app/user.tsx
  ...
  export default function Page() {
    ...
    return (
        ...
        Settings, {user?.emailAddresses[0].emailAddress}
         signOut()} />
        {userOwnsCredentials && (
           clearCredentials()} />
        )}
  ...
  ## Update credentials while user is signed in
