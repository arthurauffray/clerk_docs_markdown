...
> Learn how to add geo blocking to your Next.js + Clerk application.
...
#### Render
    ...
    The following example shows how to implement geo blocking in your [`clerkMiddleware()`](/reference/nextjs/clerk-middleware) using Render's `cf-ipcountry` header. If a user visits any route that is not a `/block` route, the middleware checks the client's country and redirects to the `/block` route if the country is not allowed. If that route is protected, the middleware also checks if the user is authenticated.
