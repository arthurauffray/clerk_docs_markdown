...
> Learn how to use your production Clerk instance locally for debugging.
...
- A [production domain already configured](/guides/development/deployment/production) in your [Clerk Dashboard](https://dashboard.clerk.com/~/domains)
...
However, [authentication across subdomains works by default](/guides/development/deployment/production#authentication-across-subdomains) in Clerk production instances. By mapping a subdomain of your production domain to your local machine, you can use production keys locally.
...
> - **Session behavior differs** - Production and development instances have [different session architectures](/guides/development/managing-environments#session-architecture-differences). Some issues may still not reproduce locally.
...
#### Windows
      ...
      1. In Notepad, select **File > Open** and navigate to `C:\Windows\System32\drivers\etc\hosts`. You may need to change the file filter from "Text Documents" to "All Files" to see it.
...
// Filename: /etc/hosts
         ...
         127.0.0.1 local.your-domain.com
         ```
      1. Save and exit (in nano, press `Ctrl+O` to save, then `Ctrl+X` to exit).
...
1. In Notepad, select **File > Open** and navigate to `C:\Windows\System32\drivers\etc\hosts`.
1. Delete the line containing `local.your-domain.com`. 1. Save the file.
