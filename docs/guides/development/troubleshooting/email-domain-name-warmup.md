...
> Learn how email providers evaluate new domains, how to warm up your domain for reliable verification email delivery, and what metrics to monitor as a Clerk customer.

## Why your domain reputation matters

Clerk automatically warms the dedicated IP addresses used to send emails, but IP reputation is only half the equation. Email providers also evaluate the reputation of the domain in the `From:` header of every message — and that domain is yours.
...
- **Authentication records** — Whether your SPF, DKIM, and DMARC records are properly configured and passing. Clerk configures SPF and DKIM for you during production setup. You should also [set up DMARC](#dmarc) yourself. For more on authentication records, see [email deliverability](/guides/development/troubleshooting/email-deliverability#spf-and-dkim-email-authentication).
...
- **Gmail** — The most transparent provider. [Postmaster Tools](https://gmail.com/postmaster/) provides data on your domain's reputation and spam rate. Gmail requires SPF, DKIM, and DMARC for bulk senders (5,000+ messages/day to Gmail accounts). Non-compliant emails face temporary or permanent rejection. Gmail also uses "pre-delivery message scanning" for Google Workspace accounts, which can delay messages by roughly 4 minutes. See [known provider-related issues](/guides/development/troubleshooting/email-deliverability#known-provider-related-issues).
...
Clerk configures SPF and DKIM records for your domain during production instance setup. You should also [set up DMARC yourself](#dmarc), which Clerk recommends but cannot configure automatically.

> [!IMPORTANT]
> For detailed information on SPF, DKIM, and DMARC configuration, see [email deliverability](/guides/development/troubleshooting/email-deliverability). The sections below cover warm-up-specific considerations only.
...
For more on DMARC setup, see [setup DMARC email authentication](/guides/development/troubleshooting/email-deliverability#setup-dmarc-email-authentication).

### Reverse DNS (PTR records)

Your sending IP address should have a valid PTR record that resolves back to a hostname, and that hostname should resolve forward to the same IP. This round-trip verification is called forward-confirmed reverse DNS (FCrDNS) — it proves that the owner of the IP address and the owner of the hostname are the same entity. Gmail requires FCrDNS for all senders, and other major providers use it as a signal when evaluating sender trustworthiness.

Clerk's email sending infrastructure handles PTR records automatically. You don't need to configure this yourself unless you're [managing your own email delivery](/guides/development/troubleshooting/email-deliverability#managing-your-own-email-delivery).

### TLS

All email sent to Gmail must use a TLS-encrypted connection. Clerk sends all email over TLS by default.
...
### Before you send anything
  ...
  SPF and DKIM are configured during Clerk production setup, but DMARC is your responsibility. Do this before you send your first production email.
...
- Starting your DMARC policy at `p=none` so you can monitor without risking rejections
...
- **Skipping DMARC** — Clerk sets up SPF and DKIM, but DMARC is your responsibility. Without it, you don't meet Gmail's bulk sender requirements, and your emails can be rejected outright.
...
- **Ignoring Outlook** — Microsoft's filters are more aggressive and less transparent than Gmail's. If your users are on Outlook or Office365 (common for B2B apps), test deliverability there specifically. Verification codes tend to have better deliverability with Outlook. See [known provider-related issues](/guides/development/troubleshooting/email-deliverability#known-provider-related-issues).

## Related guides


  - [Email deliverability](/guides/development/troubleshooting/email-deliverability)
  - Learn about SPF, DKIM, DMARC setup, and managing your own email delivery.

  ---

  - [Known provider-related issues](/guides/development/troubleshooting/email-deliverability#known-provider-related-issues)
