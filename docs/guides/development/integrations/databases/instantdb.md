...
> Learn how to integrate Clerk into your InstantDB application.
...
> Examples for this SDK aren't available yet. For now, try adapting the  available example to fit your SDK.
  ...
  InstantDB uses Clerk's [session token](/guides/sessions/session-tokens) to authenticate users. To use InstantDB with Clerk, you need to include the `email` claim in your Clerk session token.
