...
> [!TIP]
> This integration restricts what data authenticated users can access in the database, but does not synchronize user records between Clerk and Supabase. To send additional data from Clerk to your Supabase database, use [webhooks](/guides/development/webhooks/overview).
...
#### Client-side rendering
      ...
      The following example uses the [Next.js SDK](/reference/nextjs/overview) to access the [`useUser()`](/reference/hooks/use-user) and [`useSession()`](/reference/hooks/use-session) hooks, but you can adapt this code to work with any React-based Clerk SDK.
...
#### Server-side rendering
      ...
      The following example uses the [Next.js SDK](/reference/nextjs/overview) to demonstrate how to integrate Supabase with Clerk in a **server-side rendered** application.

      The `createServerSupabaseClient()` function is stored in a separate file so that it can be re-used in multiple places, such as within `page.tsx` or a Server Action file. This function uses the [`auth().getToken()`](/reference/nextjs/app-router/auth#data-fetching-with-get-token) method to pass the Clerk session token to the Supabase client.

      ```ts
// Filename: app/ssr/client.ts
...
```ts
// Filename: app/ssr/AddTaskForm.tsx
