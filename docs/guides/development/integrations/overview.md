# Integrations


> Learn about the available integrations with Clerk.

## Connect with databases and more


  - [Vercel Marketplace](/guides/development/integrations/platforms/vercel-marketplace)
  - Configure and integrate Clerk into your Vercel projects through the Vercel Marketplace.
  - ![Vercel logo](/images/logos/auth_providers/vercel.svg){{ dark: '/docs/images/logos/auth_providers/vercel-dark.svg' }}

  ---

  - [Convex](/guides/development/integrations/databases/convex)
  - Clerk offers a first-class integration with Convex, the serverless state management platform.
  - ![Convex logo](/images/logos/convex.svg)

  ---

  - [Firebase](/guides/development/integrations/databases/firebase)
  - Clerk integrates directly with Firebase, a popular Platform-as-a-Service (PaaS) provider, so developers can easily add our beautiful Sign Up, Sign In, and User Profile UIs to their Firebase application.
  - ![Firebase logo](/images/logos/firebase.svg)

  ---

  - [Google Analytics](/guides/development/integrations/analytics/google-analytics)
  - Send user authentication events to your Google Analytics property to enable metrics tracking on user sign up and sign in events.
  - ![Google Analytics logo](/images/logos/google-analytics.svg)

  ---

  - [Grafbase](/guides/development/integrations/databases/grafbase)
  - Build applications with serverless GraphQL backends provided by Grafbase and user authentication provided by Clerk.
  - ![Grafbase logo](/images/logos/grafbase.svg){{ dark: '/docs/images/logos/grafbase-dark.svg' }}

  ---

  - [Hasura](/guides/development/integrations/databases/hasura)
  - Easily connect to your Hasura GraphQL endpoint using Clerk as your authentication provider.
  - ![Hasura logo](/images/logos/hasura.svg)

  ---

  - [Inngest](/guides/development/webhooks/inngest)
  - Use events from Clerk to trigger functions defined in your codebase with Inngest. Easily handle data synchronization, onboarding campaigns, or billing workflows in code.
  - ![Inngest logo](/images/logos/inngest.svg){{ dark: '/docs/images/logos/inngest-dark.svg' }}

  ---

  - [Instant](/guides/development/integrations/databases/instantdb)
  - Clerk has a default integration with Instant. You get all the auth features of Clerk, and a relational database you can use directly from the browser.
  - ![Instant logo](/images/logos/instant.svg)

  ---

  - [Neon](/guides/development/integrations/databases/neon)
  - Build applications using Neon's serverless Postgres database with Clerk as your authentication provider.
  - ![Neon logo](/images/logos/neon.svg)

  ---

  - [Nhost](/guides/development/integrations/databases/nhost)
  - The open source alternative to Firebase with GraphQL seamlessly integrates with Clerk to provide authenticated queries.
  - ![Nhost logo](/images/logos/nhost.svg)

  ---

  - [Prisma Postgres](/guides/development/integrations/databases/prisma-postgres)
  - Build applications using Prisma Postgres with Clerk as your authentication provider.
  - ![Prisma logo](/images/logos/prisma.svg){{ dark: '/docs/images/logos/prisma-dark.svg' }}

  ---

  - [Shopify](/guides/development/integrations/platforms/shopify)
  - Use Clerk as your preferred Auth solution for your Shopify store.
  - ![Shopify logo](/images/logos/shopify.svg)

  ---

  - [Supabase](/guides/development/integrations/databases/supabase)
  - Clerk embraces the open source alternative to Firebase and offers a first-class integration with Supabase.
  - ![Supabase logo](/images/logos/supabase.svg)


  - [Join our Discord](https://clerk.com/discord 'Join Discord')
  - Join our official Discord server to chat with us directly and become a part of the Clerk community.
  - ![Discord logo](/images/logos/auth_providers/discord.svg){{ dark: '/docs/images/logos/auth_providers/discord-dark.svg' }}

  ---

  - [Need help?](/support 'Get help')
  - Contact us through Discord, Twitter, or email to receive answers to your questions and learn more about Clerk.
  - {<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" fill="none"><g clipPath="url(#a)"><path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M22 30.603c-5.109 0-9.25-3.773-9.25-8.427 0-4.653 4.141-8.426 9.25-8.426s9.25 3.773 9.25 8.426c0 1.864-.664 3.587-1.789 4.982.28 1.476.996 3.488 1.789 4.092 0 0-2.735-.379-4.79-1.69A9.9 9.9 0 0 1 22 30.603"/><path fill="currentColor" fillOpacity=".15" fillRule="evenodd" d="M14.55 27.172c-1.131-1.398-1.8-3.125-1.8-4.996 0-4.653 4.141-8.426 9.25-8.426 3.67 0 6.84 1.947 8.335 4.768.592-1.412.915-2.937.915-4.527C31.25 6.678 24.422.75 16 .75S.75 6.678.75 13.99c0 2.93 1.095 5.636 2.95 7.829-.462 2.319-1.643 5.481-2.95 6.43 0 0 4.511-.595 7.9-2.654a16.9 16.9 0 0 0 5.9 1.577" clipRule="evenodd"/><path fill="currentColor" d="m14.55 27.172-.06.748a.75.75 0 0 0 .643-1.22zm15.785-8.654-.663.351a.75.75 0 0 0 1.355-.06zM3.7 21.818l.736.147a.75.75 0 0 0-.163-.63zM.75 28.25l-.44-.607a.75.75 0 0 0 .538 1.35zm7.9-2.655.323-.677a.75.75 0 0 0-.713.036zm6.483 1.105c-1.031-1.275-1.633-2.837-1.633-4.524H12c0 2.055.736 3.947 1.967 5.468zM13.5 22.176c0-4.175 3.738-7.676 8.5-7.676V13c-5.456 0-10 4.044-10 9.176zM22 14.5c3.408 0 6.315 1.807 7.672 4.37l1.326-.703C29.366 15.087 25.93 13 22 13zm9.027 4.309c.642-1.525.973-3.164.973-4.818h-1.5c0 1.455-.292 2.896-.857 4.237zM32 13.991C32 6.168 24.734 0 16 0v1.5c8.111 0 14.5 5.688 14.5 12.49zM16 0C7.266 0 0 6.168 0 13.99h1.5C1.5 7.189 7.889 1.5 16 1.5zM0 13.99c0 3.123 1.169 5.998 3.127 8.313l1.145-.969c-1.75-2.07-2.772-4.608-2.772-7.343zm2.964 7.682a19 19 0 0 1-1.111 3.58C1.34 26.438.785 27.296.309 27.641l.882 1.215c.831-.604 1.519-1.802 2.04-3.013.538-1.25.963-2.671 1.204-3.879l-1.471-.293zM.75 28.25l.098.743h.006l.011-.002.201-.03a28 28 0 0 0 2.553-.541c1.584-.415 3.637-1.101 5.42-2.185l-.78-1.281c-1.605.976-3.501 1.617-5.02 2.015a27 27 0 0 1-2.578.536l-.008.001zm7.576-1.979a17.6 17.6 0 0 0 6.164 1.649l.122-1.495a16.1 16.1 0 0 1-5.639-1.507z"/></g><defs><clipPath id="a"><path fill="currentColor" d="M0 0h32v32H0z"/></clipPath></defs></svg>}
