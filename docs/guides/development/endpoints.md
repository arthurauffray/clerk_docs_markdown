...
> Learn how to use Clerk's Astro SDK in your Astro endpoints.
...
If you aren't protecting your endpoints using [`clerkMiddleware()`](/reference/astro/clerk-middleware), you can use the [`auth()` local](/reference/astro/locals) and check for the `isAuthenticated` value, as shown in the following example:
...
To retrieve a token from a JWT template and fetch data from an external source, use the [`getToken()`](/reference/backend/types/auth-object#get-token) method from the [`auth()` local](/reference/astro/locals), as shown in the following example:
...
In some cases, you might need the current user in your endpoint. Use the asynchronous [`currentUser()` local](/reference/astro/locals#locals-current-user) to retrieve the current [`Backend User`](/reference/backend/types/backend-user) object, as shown in the following example:
