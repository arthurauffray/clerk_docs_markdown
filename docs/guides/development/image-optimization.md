...
> Learn how to use image optimization when fetching user images from Clerk.
...
Some types in Clerk's JavaScript SDK, such as [`User`](/reference/javascript/user), [`PublicUserData`](/reference/javascript/types/public-user-data#properties), and [`Organization`](/reference/javascript/organization#properties), have an `imageUrl`. The URL returned from this property can be scaled down with the following image optimization options:
...
On the client-side, you can use the [`useClerk()`](/reference/hooks/use-clerk) hook to get the `imageUrl` from the currently active user's `User` object.
...
#### Server-side
    ...
    On the server-side, you can use the [`getUser()`](/reference/backend/user/get-user) method to get the `imageUrl` from the user's `User` object, and display their profile picture using Clerk's image optimization options.

    `getUser()` requires a valid user ID. You can access the current user's ID from the [`Auth`](/reference/backend/types/auth-object#how-to-access-the-auth-object) object.

    ```tsx
// Filename: app/image-optimization/page.tsx
    ...
    ```
      ...
      The Next.js SDK offers the [`currentUser()`](/reference/nextjs/app-router/current-user) helper to get the current user's `User` object. This is more efficient than using the [`getUser()`](/reference/backend/user/get-user) method directly as it abstracts the user ID retrieval and user object fetching into a single helper.
...
// Filename: app/image-optimization/page.tsx
      ...
      The Astro SDK offers the [`locals.currentUser()`](/reference/astro/locals) local to get the current user's `User` object. This is more efficient than using the [`getUser()`](/reference/backend/user/get-user) method directly as it abstracts the user ID retrieval and user object fetching into a single local.

      ```astro
// Filename: src/pages/image-optimization.astro
