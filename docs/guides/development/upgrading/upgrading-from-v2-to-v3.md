...
> Learn how to upgrade from version 2 to version 3 of the Clerk JavaScript libraries.
...
- `` and `` in a way that requires extra components
...
Previously, `useUser` and `useSession` could only be called from inside the `` component. This restriction has been relaxed, but now developers must manually handle the possibility the user and session objects may not be loaded when the hook is called.

In a future version, we plan to support React `` to make this even easier.
...
`useSignUp()` and `useSignIn()` are no longer required to be contained in `` or ``. Instead, they have been updated to include a loading state.
