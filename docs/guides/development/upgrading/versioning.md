...
> A brief guide on how versioning works in Clerk.
...
Brings more consistency and clarity to the Clerk Billing API endpoints. See the [migration guide](/guides/development/upgrading/upgrade-guides/2025-11-10) for more details.
...
Adds support for [version 2 of Clerk session tokens](/guides/sessions/session-tokens).
The following SDKs are compatible with this version:
...
> [!NOTE]
> Future time ranges are listed when a specific target date is not yet determined. The version "Core `N`" refers to [how Clerk SDKs are versioned and released](/guides/development/upgrading/versioning).
