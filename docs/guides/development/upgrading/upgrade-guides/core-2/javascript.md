...
> Learn how to upgrade Clerk's Javascript SDK to the latest version.
...
The experimental property `Clerk.experimental_canUseCaptcha` has been removed. There is no replacement for this functionality currently. If this is problematic for your application, [contact support](/contact/support)!


  
**Details**

The experimental property `Clerk.experimental_captchaURL` has been removed. There is no replacement for this functionality currently. If this is problematic for your application, [contact support](/contact/support)!


  
**Details**

The experimental property `Clerk.experimental_captchaSiteKey` has been removed. There is no replacement for this functionality currently. If this is problematic for your application, [contact support](/contact/support)!


  
**Details**

The `Clerk.__unstable__invitationUpdate` experimental property has been removed, and has no current replacement. If this is an issue in your codebase, [contact support](/contact/support)!


  
**Details**

The `Clerk.__unstable__membershipUpdate` experimental property has been removed, and has no current replacement. If this is an issue in your codebase, [contact support](/contact/support)!


  
**Details**

The param `redirect_url` of [`User.createExternalAccount`](/reference/javascript/user#create-external-account) has been updated to `redirectUrl`. This is a simple text replacement without any signature changes.


  
**Details**

The `generatedSignature` param to [`Signup.attemptWeb3WalletVerification()`](/reference/javascript/sign-up#attempt-web3-wallet-verification) has been removed. Instead, use the `signature` param. Note that this param takes a string, where the `generatedSignature` param took a function, so both key and value will need to change.
...
The `profileImageUrl` property of any [`User` object](/reference/javascript/user) or [`OrganizationMembershipPublicData` object](https://github.com/clerk/javascript/blob/37f36e538d8879981f76f4a433066e057afb06de/packages/backend/src/api/resources/OrganizationMembership.ts#L31) has been changed to `imageUrl`.


  
**Details**

The `getOrganizationMemberships` [method on the `Clerk` class](/reference/javascript/clerk) has been removed. Instead, use `getOrganizationMemberships` on a user instance.
...
If you are using [`Clerk.addListener`](/reference/javascript/clerk#add-listener) or `OrganizationContext` and utilizing either the `lastOrganizationInvitation` and/or `lastOrganizationMember` emitted events, these properties have been removed, as they were only relevant for internal use. We are not providing a way to backfill this functionality at the moment. If you are using it and this is an issue for you, [contact support](/contact/support) and we can find a way to resolve the issue!
...
The `Clerk.redirectToHome` method has been removed. If you are looking for a generic replacement for this method, you can instead use `window.Clerk.redirectToAfterSignUp()` or `window.Clerk.redirectAfterSignIn()`.
    ...
    - If using a React-based SDK, pass the desired values into [``](/reference/components/clerk-provider#properties).
