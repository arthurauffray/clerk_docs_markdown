...
> Learn how to upgrade to the latest version of Clerk's SDKs

> [!NOTE]
> If you need to reference the previous documentation while upgrading, the [Core 1 docs](/core-1) are still available.
...
- 📊 [Telemetry](/guides/how-clerk-works/security/clerk-telemetry) to allow us to better determine feature usage and priority (easy opt-out if you don't like this)
...
The "Settings" tab within the `` component has been renamed to "General". If you are linking directly to `/organization-settings` from anywhere, the link will need to be updated to `/organization-general`.

    If you are [customizing the component](/guides/customizing-clerk/adding-items/organization-profile) by re-ordering the pages, the label used to target this page must be changed from `settings` to `general` as well.

    If you are using the [appearance prop](/guides/customizing-clerk/appearance-prop/overview) to customize the appearance of the `` component, note that the `cl-profilePage__organizationSettings` key has also been changed to `cl-profilePage__organizationGeneral` to be consistent with the naming change.


  
**Details**

Within the `` component (which is also rendered by ``), the "Account" and "Security" pages now live within their own tabs. Previously, "Security" was a section within the "Account" tab. The "Security" page can now be directly linked to under the `/security` path if desired.


  
**Details**

Within the `` component (which is also rendered by ``), adding a connected account is now rendered as a dropdown instead of a modal. If you were relying on the modal for any sort of customizations, these customizations will need to be adjusted to the new form factor.
...
The default values of some [appearance variables](/guides/customizing-clerk/appearance-prop/variables) have changed which may impact your UI (if you are not already overriding them).
...
- [Next.js](/guides/development/upgrading/upgrade-guides/core-2/nextjs)
- [Remix](/guides/development/upgrading/upgrade-guides/core-2/remix)
- [Expo](/guides/development/upgrading/upgrade-guides/core-2/expo)
- [Node](/guides/development/upgrading/upgrade-guides/core-2/node)
- [Fastify](/guides/development/upgrading/upgrade-guides/core-2/fastify)
- [React](/guides/development/upgrading/upgrade-guides/core-2/react)
- [Backend](/guides/development/upgrading/upgrade-guides/core-2/backend)
- [JS](/guides/development/upgrading/upgrade-guides/core-2/javascript)
- [Chrome Extension](/guides/development/upgrading/upgrade-guides/core-2/chrome-extension)
