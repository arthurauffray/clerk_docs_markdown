...
> Update imports, configuration, environment wiring, and sign-in code when moving to v1.
...
## Update your package dependency
  ...
  1. Open the **Package Dependencies** tab and select the **+** button.
  ...
  1. In **Choose Package Products**, select `ClerkKit` (and `ClerkKitUI` only if you want to use Clerk's [prebuilt views](/reference/views/overview)), then select **Add Package**.
...
// Filename: Package.swift
  ...
  v1 splits imports by product: use `ClerkKit` for core APIs, and add `ClerkKitUI` for [`AuthView`](/reference/views/authentication/auth-view), [`UserProfileView`](/reference/views/user/user-profile-view), and other [prebuilt views](/reference/views/overview).
  ...
  `Clerk.Settings` is now `Clerk.Options`. See the [iOS configuration reference](/reference/native-mobile/configuration#configuration-options) for the full list of options.
  ...
  See [Authentication flows](/reference/native-mobile/auth) for more authentication options and examples.
