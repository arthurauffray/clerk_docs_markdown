...
> Learn how to add a custom sign-in-or-up page to your Next.js app with Clerk's prebuilt components.
...
To set up separate sign-in and sign-up pages, follow this guide, and then follow the [custom sign-up page guide](/nextjs/guides/development/custom-sign-up-page).

> [!NOTE]
> Just getting started with Clerk and Next.js? See the [quickstart tutorial](/nextjs/getting-started/quickstart)!


  ## Build a sign-in-or-up page

  The following example demonstrates how to render the [``](/reference/components/authentication/sign-in) component on a dedicated page using the [Next.js optional catch-all route](https://nextjs.org/docs/pages/building-your-application/routing/dynamic-routes#catch-all-segments).
...
// Filename: app/sign-in/[[...sign-in]]/page.tsx
  ...
  By default, `clerkMiddleware()` makes all routes public. **This step is specifically for applications that have configured `clerkMiddleware()` to make [all routes protected](/reference/nextjs/clerk-middleware#protect-all-routes).** If you have not configured `clerkMiddleware()` to protect all routes, you can skip this step.
  ...
  - Create a new [route matcher](/reference/nextjs/clerk-middleware#create-route-matcher) that matches the sign-in route, or you can add it to your existing route matcher that is making routes public.
  - Create a check to see if the user's current route is a public route. If it is not a public route, use [`auth.protect()`](/reference/nextjs/app-router/auth#auth-protect) to protect the route.
...
Learn more about Clerk components, how to use them to create custom pages, and how to use Clerk's client-side helpers using the following guides.
  ...
  - [Create a custom sign-up page](/nextjs/guides/development/custom-sign-up-page)
  ...
  - [Client-side helpers](/reference/nextjs/overview#client-side-helpers)
