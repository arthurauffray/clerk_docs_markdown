...
> Learn how to integrate Clerk with Inngest.

Webhooks allow you to [synchronize data](/guides/development/webhooks/syncing) from Clerk to your application backend. You can either handle them directly in your backend with an endpoint or use a tool like [Inngest](https://www.inngest.com) which receives the webhook events for you and reliably executes functions in your codebase. When handling webhooks, Inngest receives the [webhook events](/guides/development/webhooks/overview#supported-webhook-events) for you and uses a built-in queue to reliably execute longer running functions with additional functionality including:
...
```typescript
// Filename: src/inngest/sync-user.ts
...
The `event` object contains all of the relevant data for the event. The `event.data` will match the `data` object from the standard Clerk webhook [payload structure](/guides/development/webhooks/overview#payload-structure). With this `clerk/user.created` event, the `event.data` will be a Clerk User json object.
...
While it is possible to add this logic at the end of your sync function as seen in the [previous section](/guides/development/webhooks/inngest#creating-a-function-to-sync-a-new-user-to-a-database), it’s better to decouple unrelated tasks into different functions so issues with one task do not affect the other ones. For example, if your email fails to send, it should not affect starting a trial for that user in Stripe.
...
```typescript
// Filename: src/inngest/welcome-emails.ts
