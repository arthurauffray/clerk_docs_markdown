...
> Learn about various ways to make authenticated requests to the backend when using Clerk SDKs.
...
The following headers are required for Clerk to authenticate a request. It contains information that Clerk uses to determine whether a request is in a signed in or signed out state, or if a [handshake](/guides/how-clerk-works/overview#the-handshake) must be performed.
...
> [!NOTE]
> Your application must be wrapped in a `` component with a configured `QueryClient` instance. See the [TanStack Query docs](https://tanstack.com/query/v4/docs/react/quick-start) for setup instructions.
...
If your client and server are on different origins (e.g. making an API call to a server on `api.foo.com` from JavaScript running on a client at `foo.com`), the [session token](/guides/sessions/session-tokens) needs to be passed as a Bearer token in the Authorization header.
...
Use the [`useAuth()`](/reference/hooks/use-auth) hook to access the `getToken()` method.
...
> Your application must be wrapped in a `` component with a configured `QueryClient` instance. See the [TanStack Query docs](https://tanstack.com/query/v4/docs/react/quick-start) for setup instructions.
    ...
    Use the global `Clerk.session` object to access the [`getToken()`](/reference/javascript/session#get-token) method.
