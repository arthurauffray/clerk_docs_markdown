...
> Learn how to use Clerk's M2M tokens feature to authenticate requests between machines.
...
M2M tokens can be created as either opaque tokens (the default) or JSON Web Tokens (JWTs). JWTs don't require a network request for verification and incur charges only for creation, not verification. For a detailed comparison of the two formats, see [Token formats](/guides/development/machine-auth/token-formats).
...
To create an M2M token, call the [`createToken()`](/reference/backend/m2m-tokens/create-token) method:
...
- `tokenFormat` - The format of the token. Can be `'opaque'` (default) or `'jwt'`. See [Token formats](/guides/development/machine-auth/token-formats) for more details.
...
To retrieve a list of M2M tokens for a given machine, call the [`list()`](/reference/backend/m2m-tokens/list) method:
...
Imagine a request was made from **Machine A** to **Machine B**, and Machine B received it successfully. The next step is to verify that the token is valid and that the request is coming from a valid machine. To do so, you can use the [`verify()`](/reference/backend/m2m-tokens/verify) method:
...
To revoke an M2M token, call the [`revokeToken()`](/reference/backend/m2m-tokens/revoke-token) method:
