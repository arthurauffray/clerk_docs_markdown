...
> Learn what machine authentication is, why it's used, and how it works with Clerk.
...
[OAuth](https://oauth.net/2/) is a widely adopted standard designed to provide a secure way for users to grant third-party applications limited access to their data and resources without providing their login credentials directly. While OAuth includes several different flows that serve different purposes, Clerk primarily supports the [authorization code flow](/blog/how-oauth-works#the-o-auth-authorization-code-flow-in-action), which returns an OAuth access token that can be used by a machine to authenticate requests to Clerk's API on behalf of a user.

To learn more about machine authentication with OAuth access tokens, see the [OAuth scoped access](/guides/configure/auth-strategies/oauth/scoped-access) and [OAuth token verification](/guides/configure/auth-strategies/oauth/verify-oauth-tokens) guides.
...
To learn more about machine authentication with M2M tokens, see the [M2M tokens](/guides/development/machine-auth/m2m-tokens) guide.
...
With this feature, you will be able to allow users to create API keys directly through the [``](/reference/components/user/user-profile) component, or create your own UI for API keys via custom hooks. It will also be simple to verify API keys on your backend using Clerk's SDKs, and you or your users will be able to instantly revoke API keys at any time.

To learn more about machine authentication with API keys, see the [API keys](/guides/development/machine-auth/api-keys) guide.
