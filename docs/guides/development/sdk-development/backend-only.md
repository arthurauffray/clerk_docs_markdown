...
> A reference for implementing a backend-only Clerk SDK

When creating a backend-only SDK, you have two options for implementing the [BAPI](/guides/development/sdk-development/terminology) endpoints: either [develop a backend SDK that encompasses all BAPI endpoints](#implementation-bapi) or [create an SDK tailored for an existing backend framework](#implementation-node-js-backend-framework).

The source of truth for all BAPI endpoints is the [BAPI reference docs](/reference/backend-api). For Node.js backend frameworks, you can build on top of the [JS Backend SDK](/js-backend/getting-started/quickstart).
...
> [!IMPORTANT]
> BAPI has [rate limits](/guides/how-clerk-works/system-limits) to help protect users against brute-force attacks or stop abuse of Clerk's platform. Be sure to include a backoff mechanism into your fetching logic and respect the `Retry-After` header to gracefully handle any active rate limits.
...
You can manually create a wrapper library around the [BAPI OpenAPI](/reference/backend-api) or use one the many automatic SDK generation tools that take in OpenAPI definitions.
...
> The code blocks below will be written in pseudo-code. If you're looking for real-world examples, have a look at [`@clerk/fastify`](https://github.com/clerk/javascript/tree/main/packages/fastify) and [`@clerk/express`](https://github.com/clerk/javascript/tree/main/packages/express).
  ...
  Use [`createClerkClient`](/getting-started/quickstart#usage) from `@clerk/backend` to create your default Clerk client which will be used for the middleware.
...
// Filename: client.ts
  ...
  Inside the middleware, you’ll use the user-provided Clerk client (or use the default one created in the previous step) and authenticate the request. [`authenticateRequest`](/reference/backend/authenticate-request) returns `Promise`. The middleware should set `requestState.toAuth()` into its context as this will contain the resolved signed-in/signed-out [`Auth`](/reference/backend/types/auth-object) object. This way other helpers can access it later in the chain.
...
// Filename: index.ts
  ...
  > [!TIP]
  > See the Next.js [`getAuth()`](/reference/nextjs/pages-router/get-auth) reference to see how it's implemented.
...
// Filename: index.ts
  ...
  Your end-users will also have access to a [`has()`](/reference/backend/types/auth-object#has) function on the [`Auth`](/reference/backend/types/auth-object) object. They can combine it with `requireAuth()` like so:
