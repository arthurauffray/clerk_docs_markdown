...
> Learn which Clerk environment variables are necessary to configure your projects.
...
If you're building a pure React app, you should use the props on the components you're using. For example, to force users to redirect to a specific URL after signing in, you would use the `signInForceRedirectUrl` prop on [``](/reference/components/clerk-provider) rather than the `CLERK_SIGN_IN_FORCE_REDIRECT_URL` environment variable.

## Sign-in and sign-up redirects

Components, such as [``](/reference/components/clerk-provider), [``](/reference/components/authentication/sign-in), and more, provide props for you to specify where users will be redirected. For example, `` has the `signInFallbackRedirectUrl` and `signUpFallbackRedirectUrl` props.
...
| Variable | Description |
| - | - |
| `VITE_CLERK_PUBLISHABLE_KEY` | Your Clerk app's Publishable Key. It will be prefixed with `pk_test_` in development instances and `pk_live_` in production instances. |
...
| `CLERK_JWT_KEY` | Sets the JWT verification key that Clerk will use to provide networkless JWT session token verification. Refer to [Manual JWT verification](/guides/sessions/manual-jwt-verification). |
...
| `CLERK_ENCRYPTION_KEY` | Sets the encryption key to securely propagate `clerkMiddleware` dynamic keys during request time. A 128-bit, pseudorandom value should be used. See [Dynamic keys](/reference/nextjs/clerk-middleware#dynamic-keys) to learn more. |
| `CLERK_JWT_KEY` | Sets the JWT verification key that Clerk will use to provide networkless JWT session token verification. Refer to [Manual JWT verification](/guides/sessions/manual-jwt-verification). |
...
Clerk supports sharing sessions across different domains by adding one or many satellite domains to an application. See [the satellite domains guide](/guides/dashboard/dns-domains/satellite-domains) for more information.
...
The following environment variable allows you to protect your webhook signing secret. It is read by Clerk's [`verifyWebhook()`](/reference/backend/verify-webhook) function.
...
Clerk provides environment variables for opting out of telemetry data collection. See [the telemetry documentation](/guides/how-clerk-works/security/clerk-telemetry) for more information.
