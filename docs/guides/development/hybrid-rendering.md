...
> Clerk's Astro SDK supports Astro's on-demand rendering output modes (server and hybrid).
...
For example, say you were trying to align the content inside a `` component using `flex`, as shown in the following code:
...
The `clerk-signed-in` wrapper wouldn't be a flex item by default, potentially misaligning the content. To fix this, you would need to apply flex properties to the `` component itself by using a selector, as shown in the following example:
