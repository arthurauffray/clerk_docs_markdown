...
> [!WARNING]
> This guide is for creating custom JWT templates in order to generate JSON Web Tokens with Clerk. If you are looking for how to customize your Clerk-generated session token, refer to the [Customize your session token](/guides/sessions/customize-session-tokens) guide.
...
In every generated token, there are certain claims that are automatically included and cannot be overridden by templates. Clerk calls these "default claims" and you can learn more about them in the [Session tokens](/guides/sessions/session-tokens) reference documentation.
...
If you need to generate a token that includes both session-bound data (like `sid`) and any additional claims, the recommended approach is to use a custom session token instead of a JWT template. See the [guide on customizing your Clerk session token](/guides/sessions/customize-session-tokens).
...
To generate a token using a template, you can use the `getToken()` method. See the [reference documentation](/reference/backend/types/auth-object#get-token) for more information and example usage.
