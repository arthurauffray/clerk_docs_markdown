# Custom metadata for B2B authentication flows


> Learn how to add custom metadata to your B2B authentication flows to store additional information in the org object for advanced user segmentation, analytics, and B2B workflows.
...
Metadata is stored on the [`Organization`](/reference/javascript/organization) and [`OrganizationMembership`](/reference/javascript/types/organization-membership) objects.
...
Both the [`Organization`](/reference/javascript/organization) and [`OrganizationMembership`](/reference/javascript/types/organization-membership) objects have the metadata fields: `publicMetadata` and `privateMetadata`.
...
You can set Organization metadata in the [Clerk Dashboard](https://dashboard.clerk.com/~/organizations) or using Clerk's Backend API. See the [`updateOrganizationMetadata()`](/reference/backend/organization/update-organization-metadata) and [`updateOrganizationMembershipMetadata()`](/reference/backend/organization/update-organization-membership-metadata) methods for more information.

## Access public metadata

To access public metadata on the frontend, it's available on the [`Organization`](/reference/javascript/organization) object, which can be accessed using the [`useOrganization()`](/reference/hooks/use-organization) hook.

To access public metadata on the backend, it's available on the [Backend `Organization`](/reference/backend/types/backend-organization) object which can be accessed using the [`getOrganization()`](/reference/backend/organization/get-organization) method. This method will return the `Organization` object which contains the public metadata. However, this method is subject to [rate limits](/guides/how-clerk-works/system-limits#backend-api-requests), so _if you are accessing the metadata frequently_, it's recommended to [attach it to the user's session token](#metadata-in-the-session-token).
...
Now that you understand Organization metadata, you can:
  ...
  - [Add metadata to invitations](/guides/organizations/add-members/invitations#invitation-metadata)
