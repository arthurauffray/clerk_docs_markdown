# Set up Organization-level SAML and OIDC for B2B/B2C apps


> Integrate as many Enterprise SSO methods within Clerk Organizations. Enable SAML SSO, OAuth/OIDC, and other secure MFA/single sign-on options for B2B SaaS apps.

Clerk provides Enterprise Single Sign-On (SSO) through a feature called [**Enterprise Connections**](/guides/configure/auth-strategies/enterprise-connections/overview). You can enable Enterprise Connections for specific Organizations, allowing members to authenticate through their company's identity provider using SAML or OIDC protocols.

When users sign up or sign in using an Organization's Enterprise Connection, Clerk automatically adds them as members of that Organization and assigns them the [default Role](/guides/organizations/control-access/roles-and-permissions#the-default-role-for-members). This process is known as [Just-in-Time (JIT) provisioning](/guides/configure/auth-strategies/enterprise-connections/jit-provisioning).
...
1. [Create an Organization](/guides/organizations/create-and-manage#create-an-organization) for your customer through the Clerk Dashboard.
...
1. After adopting the application, the user [creates an Organization](/guides/organizations/create-and-manage#create-an-organization) for their company.
...
Clerk supports Enterprise SSO via [SAML](/guides/configure/auth-strategies/enterprise-connections/overview#saml) or via the [OpenID Connect (OIDC) protocol](/guides/configure/auth-strategies/enterprise-connections/overview#oidc), either through EASIE or by integrating with any OIDC-compatible provider.

To add an Enterprise SSO Connection for an Organization, go to the [Enterprise Connections](/guides/configure/auth-strategies/enterprise-connections/overview) docs and follow the appropriate guide based on the platform you want to use, such as the [Google SAML guide](/guides/configure/auth-strategies/enterprise-connections/saml/google). When configuring the connection in the Clerk Dashboard, there will be an option to select the **Organization** for which you want to enable this connection. If you don't select an Organization, Clerk will add the connection for your entire application.

> [!WARNING]
> A domain used for Enterprise SSO can't be used as a [Verified Domain](/guides/organizations/add-members/verified-domains) for the same Organization.
...
Now that you've set up Enterprise SSO, you can:
  ...
  - [JIT provisioning](/guides/configure/auth-strategies/enterprise-connections/jit-provisioning)
  - Learn more about JIT provisioning to customize how users are automatically added to Organizations.

  ---

  - [Configure Verified Domains](/guides/organizations/add-members/verified-domains)
  - Learn how to automatically invite users who don't use SSO.

  ---

  - [Invite specific users](/guides/organizations/add-members/invitations)
  ...
  - [Configure default Roles](/guides/organizations/configure#default-roles)
