# Check Roles and Permissions in Organizations


> Limit access to content or entire routes based on a user's Organization Role or Permissions.
...
For examples on how to perform authorization checks, see the [guide on authorization checks](/guides/secure/authorization-checks).
...
#### Client-side
    ...
    - The [`Auth`](/reference/backend/types/auth-object) object is used to access the `isSignedIn` property and `has()` method.
    ...
    - The [`useOrganization()`](/reference/hooks/use-organization) hook is used to access the organization data.
    - The Organization name is checked to ensure it matches the required Organization name. If a user is not in the required Organization, the page will display a message and the [``](/reference/components/organization/organization-switcher) component will be rendered to allow the user to switch to the required Organization.
...
// Filename: app/protected/page.tsx
    ...
    For more examples on how to perform authorization checks, see the [dedicated guide](/guides/secure/authorization-checks).
...
#### Server-side
    ...
    - The [`Auth`](/reference/backend/types/auth-object) object is used to access the `isAuthenticated` and `orgId` properties, as well as the `has()` method.
    ...
    - The `has()` method is used to check if the user has the `org:admin` Role.
    - To fetch the Organization server-side, the [`clerkClient()`](/reference/nextjs/overview#clerk-client) helper is used to access the [`getOrganization()`](/reference/backend/organization/get-organization) method.
    - The Organization name is checked to ensure it matches the required Organization name. If a user is not in the required Organization, the page will display a message and the [``](/reference/components/organization/organization-switcher) component will be rendered to allow the user to switch to the required Organization.

    This example is written for Next.js App Router, but can be adapted to other frameworks by using the appropriate method for accessing the [`Auth` object](/reference/backend/types/auth-object), and the appropriate initialization for `clerkClient()`.
...
Now that you know how to check Roles and Permissions, you can:
  ...
  - [Features and Plans](/guides/billing/for-b2b#control-access-with-features-plans-and-permissions)
