...
> Clerk collects telemetry data from its SDKs about general product and feature usage.
...
- How often are our different components (``, ``, ``) rendered?
...
If you are using `@clerk/react` directly, or using an SDK that doesn't have environment variable support, you can opt out by passing the `telemetry` prop to ``:
