...
> This guide outlines a number of the common architecture scenarios for building B2B, B2C, and Platform applications with Clerk, their characteristics, and limitations.
...
- The [`` component](/reference/components/organization/organization-switcher) provides a way for your users to select which Organization is the Active Organization. The [`useOrganizationList()` hook](/guides/development/custom-flows/organizations/organization-switcher) can be used for more control.
- The [`useOrganization()` hook](/reference/hooks/use-organization) can be used to fetch the Active Organization.
- The [`` component](/reference/components/control/show) enables you to limit who can view certain pages based on their role. Additionally, Clerk exposes a number of helper functions, such as [`auth()`](/reference/nextjs/app-router/auth), and hooks, such as [`useAuth()`](/reference/hooks/use-auth), to check the user's authorization throughout your app and API endpoints.
