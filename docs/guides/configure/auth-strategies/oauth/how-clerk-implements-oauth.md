...
> Learn how to configure and secure OAuth flows with Clerk, including dynamic client registration, consent screens, PKCE, public clients, and scopes.
...
Clerk issues OAuth access tokens as [JSON Web Tokens (JWTs)](/guides/how-clerk-works/tokens-and-signatures#json-web-tokens-jwts) by default. JWTs are self-contained tokens that can be verified without making a network request to Clerk's servers.
...
For a detailed comparison of JWT and opaque token formats, including trade-offs and when to use each, see [Token formats](/guides/development/machine-auth/token-formats).
...
- `https://clerk..com/.well-known/oauth-authorization-server` for a production environment.
