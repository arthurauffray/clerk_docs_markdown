...
> Learn how to implement single sign-on with OAuth & OpenID Connect in your Clerk application or when signing into a third-party app.
...
Refer to the appropriate provider's [guide](/guides/configure/auth-strategies/social-connections/overview) for implementation details.
...
- **Support AI agents and automation** – Allow [MCP servers or AI tools](/guides/ai/overview) to authenticate users through your app.
...
1. Complete the following fields:
   ...
   - `Scopes` - The scopes that you would like to leverage. [Learn more about the scopes currently supported by Clerk](/guides/configure/auth-strategies/oauth/how-clerk-implements-oauth#scopes).
...
To learn more about the other OAuth application configuration settings, like **Dynamic client registration**, see the [dedicated guide](/guides/configure/auth-strategies/oauth/how-clerk-implements-oauth).
...
After a user has successfully completed an OAuth 2.0 flow, you can retrieve additional user information from Clerk's [`/oauth/userinfo`](/reference/frontend-api/tag/oauth2-identify-provider/post/oauth/userinfo) endpoint. When making the request to this endpoint, you must include the OAuth access token in the `Authorization` header.
...
For validating access tokens or refresh tokens and retrieving additional token metadata, Clerk provides a standard OAuth 2.0 [Token Introspection Endpoint](https://datatracker.ietf.org/doc/html/rfc7662) at [`/oauth/token_info`](/reference/frontend-api/tag/oauth2-identify-provider/post/oauth/token_info).
...
- `https://clerk..com/.well-known/jwks.json` for a production environment.
