...
1. The user **hasn't enabled [multi-factor authentication (MFA)](/guides/configure/auth-strategies/sign-up-sign-in-options#multi-factor-authentication)**.
...
If you've built a custom sign-in flow that allows password-based sign-ins using the Clerk API or SDKs, you'll need to handle the `needs_client_trust` status that Client Trust can trigger.
...
For a complete implementation example, see the [email/password custom flow guide](/guides/development/custom-flows/authentication/email-password#sign-in-flow).
