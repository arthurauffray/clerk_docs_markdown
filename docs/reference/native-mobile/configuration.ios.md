...
> Configure the Clerk iOS SDK with your publishable key and options.
...
- **`keychainConfig`** [`Clerk.Options.KeychainConfig`](#keychain-config)
...
- **`redirectConfig`** [`Clerk.Options.RedirectConfig`](#redirect-config)

  OAuth redirect URLs and callback handling.

    ---

- **`watchConnectivityEnabled`** `Bool`

  Sync auth state to a watchOS companion app. Defaults to `false`.

    ---

- **`loggerHandler`** `(@Sendable (LogEntry) -> Void)?`

  Callback for error-level SDK logs.

    ---

- **`middleware`** [`Clerk.Options.MiddlewareConfig`](#middleware-config)
...
## Next steps
  ...
  - [Clerk](/ios/reference/native-mobile/clerk)
  - Learn how to access Clerk in your app.

  ---

  - [iOS Quickstart](/ios/getting-started/quickstart)
