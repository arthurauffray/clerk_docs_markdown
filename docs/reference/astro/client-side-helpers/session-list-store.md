...
> Clerk's $sessionList store retrieves a list of sessions that have been registered on the client device.

The `$sessionListStore` store returns an array of [`Session`](/reference/javascript/session) objects that have been registered on the client device.
