...
> Clerk's $signInStore nanostore provides a convenient way to access the SignIn object, which allows you to check the current state of a sign-in.

The `$signInStore` store provides a convenient way to access the [`SignIn`](/reference/javascript/sign-in) object, which allows you to check the current state of a sign-in. This is also useful for creating a custom sign-in flow.
