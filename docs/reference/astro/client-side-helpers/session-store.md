...
> Clerk's $sessionStore nanostore provides a convenient way to access the current user Session object, as well as helpers to set the session.

The `$sessionStore` store provides a convenient way to access the current user's [`Session`](/reference/javascript/session) object, as well as helpers for setting the active session.
