...
> Clerk's <TaskSelectComponent /> component renders a UI for resolving the choose-organization task.

The `` component renders a UI for resolving the `choose-organization` session task. You can further customize your `` component by passing additional [properties](#properties) at the time of rendering.
...
Clerk's sign-in flows, such as the [Sign-in Account Portal page](/guides/account-portal/overview#sign-in), [``](/reference/components/unstyled/sign-in-button), and [``](/reference/components/authentication/sign-in) component, automatically handle the `choose-organization` session task flow for you, including rendering the `` component when needed.
...
If you want to customize the route where the `` component is rendered or customize its appearance, you can host it yourself within your application.
  ...
  The following example demonstrates how to host the `` component on a custom page. You first need to [set the `taskUrls` option on your Clerk integration](/guides/configure/session-tasks#using-the-task-urls-option) so that users are redirected to the page where you host the `` component when they have a pending `choose-organization` session task.
...
// Filename: index.tsx
    ...
    ```jsx
// Filename: src/session-tasks/choose-organization.tsx

    import { TaskChooseOrganization } from '@clerk/react'

    const ChooseOrganizationPage = () => export default ChooseOrganizationPage
    ```
...
// Filename: app/root.tsx
    ...
    ```tsx
// Filename: app/routes/session-tasks/choose-organization.tsx
...
// Filename: src/routes/__root.tsx
    ...
    ```tsx
// Filename: src/routes/session-tasks/choose-organization.tsx
  ...
  You first need to set the `taskUrls` option on the [`clerk.load()`](/reference/javascript/clerk#load) method so that users are redirected to the page where you host the `` component when they have a pending `choose-organization` session task.
...
- **`props?`** [`TaskChooseOrganizationProps`](#properties)
