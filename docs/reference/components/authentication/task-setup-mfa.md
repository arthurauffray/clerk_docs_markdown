...
Clerk's sign-in flows, such as the [Sign-in Account Portal page](/guides/account-portal/overview#sign-in), [``](/reference/components/unstyled/sign-in-button), and [``](/reference/components/authentication/sign-in) component, automatically handle the `setup-mfa` session task flow for you, including rendering the `` component when needed.
...
If you want to customize the route where the `` component is rendered or customize its appearance, you can host it yourself within your application.
  ...
  The following example demonstrates how to host the `` component on a custom page. You first need to [set the `taskUrls` option on your Clerk integration](/guides/configure/session-tasks#using-the-task-urls-option) so that users are redirected to the page where you host the `` component when they have a pending `setup-mfa` session task.
...
// Filename: app/layout.tsx
    ...
    ```tsx
// Filename: app/session-tasks/setup-mfa/page.tsx
...
// Filename: index.tsx
    ...
    ```jsx
// Filename: src/session-tasks/setup-mfa.tsx

    import { TaskSetupMFA } from '@clerk/react'

    const SetupMfaPage = () => export default SetupMfaPage
    ```
...
// Filename: app/root.tsx
    ...
    ```tsx
// Filename: app/routes/session-tasks/setup-mfa.tsx
...
// Filename: src/routes/__root.tsx
    ...
    ```tsx
// Filename: src/routes/session-tasks/setup-mfa.tsx
  ...
  You first need to set the `taskUrls` option on the [`clerk.load()`](/reference/javascript/clerk#load) method so that users are redirected to the page where you host the `` component when they have a pending `setup-mfa` session task.
...
- **`props?`** [`TaskSetupMFAProps`](#properties)
