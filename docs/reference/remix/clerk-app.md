...
- **`clerkJSVariant`** `'headless' | ''`
  ...
  If your web application only uses [control components](/reference/components/overview#control-components), you can set this value to `headless` and load a minimal ClerkJS bundle for optimal page performance.
...
- **`dynamic`** `boolean`
  ...
  (For Next.js only) Indicates whether or not Clerk should make dynamic auth data available based on the current request. Defaults to `false`. Opts the application into dynamic rendering when `true`. For more information, see [Next.js rendering modes and Clerk](/guides/development/rendering-modes).

    ---

- **`initialState`** `InitialState`

  Provide an initial state of the Clerk client during server-side rendering. You don't need to set this value yourself unless you're [developing an SDK](/guides/development/sdk-development/overview).
...
- **`nonce`** `string`
  ...
  This nonce value will be passed to the `@clerk/clerk-js` script tag. Use it to implement a [strict-dynamic CSP](/guides/secure/best-practices/csp-headers#implementing-a-strict-dynamic-csp). Requires the `dynamic` prop to also be set.
...
- **`syncHost`** `string`
  ...
  **Chrome Extension only** To enable, pass the URL of the web application that the extension will sync the authentication state from. See the [dedicated guide](/guides/sessions/sync-host) for more information.
...
- **`telemetry`** `false | { disabled: boolean; debug: boolean } | undefined`
  ...
  Controls whether or not Clerk will collect [telemetry data](/guides/how-clerk-works/security/clerk-telemetry). If set to `debug`, telemetry events are only logged to the console and not sent to Clerk.

    ---

- **`touchSession`** `boolean`

  By default, [the Clerk Frontend API `touch` endpoint](/reference/frontend-api/tag/sessions/post/v1/client/sessions/\{session_id}/touch) is called during page focus to keep the last active session alive. This option allows you to disable this behavior.

    ---

- **`waitlistUrl`** `string`

  The full URL or path to the waitlist page. If `undefined`, will redirect to the [Account Portal waitlist page](/guides/account-portal/overview#waitlist).
