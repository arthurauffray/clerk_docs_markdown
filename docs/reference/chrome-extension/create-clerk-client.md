...
> createClerkClient() allows Chrome Extension developers to initialize Clerk in popups, side panels, and background service workers without React.
...
> [!NOTE]
> If your extension uses React, use [Clerk's React hooks](/chrome-extension/reference/hooks/overview) instead. `createClerkClient()` is intended for extensions that don't use React.
...
> [!WARNING]
> The following example assumes that you have followed the [Chrome Extension Quickstart](/chrome-extension/getting-started/quickstart) and then the [add React Router](/guides/development/add-react-router) guide, but you can apply these concepts to your own application.
...
// Filename: src/tabs/background-worker-demo.tsx
     ...
     export default function NewTab() {
       ...
       const getToken = async (e: React.MouseEvent) => {
         e.preventDefault()
...
- **`syncHost?`** `string`
  ...
  The host to sync the session with. For more information, see [the dedicated guide](/guides/sessions/sync-host).
