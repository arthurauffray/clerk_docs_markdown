...
> The APIKeys object provides methods for creating, listing, and revoking API keys that allow users to grant third-party services programmatic access to your application's API endpoints.
...
Retrieves a paginated list of API keys for the current user or organization. Returns a [`ClerkPaginatedResponse`][pag-ref] of [`APIKeyResource`](/reference/javascript/types/api-key-resource) objects.
...
Creates a new API key. Returns an [`APIKeyResource`](/reference/javascript/types/api-key-resource) object that includes the `secret` property. **The secret is only available in the response from `create()` and cannot be retrieved later.**
...
- **`subject?`** `string`
  ...
  The user or organization ID to associate the API key with. If not provided, defaults to the Active Organization, then the current User.
...
Revokes an API key by ID. Returns an [`APIKeyResource`](/reference/javascript/types/api-key-resource) object.
