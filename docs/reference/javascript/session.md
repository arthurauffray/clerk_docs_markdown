...
As soon as a [`User`](/reference/javascript/user) signs in, Clerk creates a `Session` for the current [`Client`][client-ref]. Clients can have more than one sessions at any point in time, but only one of those sessions will be **active**.

In certain scenarios, a session might be replaced by another one. This is often the case with [multi-session applications](/guides/secure/session-options#multi-session-applications).

All sessions that are **expired**, **removed**, **replaced**, **ended** or **abandoned** are not considered valid.

> [!NOTE]
> For more information regarding the different session states, see the [guide on session management](/guides/secure/session-options).
...
- **`actor`** `ActJWTClaim | null`
  ...
  The JWT actor for the session. Holds identifier for the user that is impersonating the current user. Read more about [impersonation](/guides/users/impersonation).
...
- **`currentTask`** <code>[SessionTask](/reference/javascript/types/session-task) | undefined</code>

  The current pending task for the session. Read more about [session tasks](/guides/configure/session-tasks).
...
- **`status`** [`SessionStatus`](/reference/javascript/types/session-status)

  The current state of the session.

    ---

- **`tasks`** <code>[SessionTask](/reference/javascript/types/session-task)\[] | null</code>
...
- **`user`** [`User`](/reference/javascript/user)
...
Attempts to complete the first factor verification process. Returns a [`SessionVerification`](/reference/javascript/types/session-verification) instance with its status and supported factors.
...
Attempts to complete the second factor verification process. Returns a [`SessionVerification`](/reference/javascript/types/session-verification) instance with its status and supported factors.
...
Checks if the user is [authorized for the specified Role, Permission, Feature, or Plan](/guides/secure/authorization-checks) or requires the user to [reverify their credentials](/guides/secure/reverification) if their last verification is older than allowed.
...
- **`role`** `string`
  ...
  Accepts [Role](/guides/organizations/control-access/roles-and-permissions#roles) key.

    ---

- **`permission`** `string`

  Accepts [Permission](/guides/organizations/control-access/roles-and-permissions#permissions) key.

    ---

- **`feature`** `string`

  Accepts [Feature](/guides/secure/features) key.

    ---

- **`plan`** `string`

  Accepts [Plan](/guides/billing/overview) key.

    ---

- **`reverification?`** <code>[ReverificationConfig](/reference/backend/types/auth-object#reverification-config)</code>
...
For more information, see the [guide on authorization checks](/guides/secure/authorization-checks).
...
For more information, see the [guide on reverification](/guides/secure/reverification).
...
Retrieves the current user's [session token](/guides/sessions/session-tokens) or a [custom JWT template](/guides/sessions/jwt-templates).
...
Initiates the first factor verification process. This is a required step to complete a reverification flow when using a preparable factor. Returns a [`SessionVerification`](/reference/javascript/types/session-verification) instance with its status and supported factors.
...
Initiates the second factor verification process. This is a required step to complete a reverification flow when using a preparable factor. Returns a [`SessionVerification`](/reference/javascript/types/session-verification) instance with its status and supported factors.
...
Initiates the reverification flow. Returns a [`SessionVerification`](/reference/javascript/types/session-verification) instance with its status and supported factors.

```typescript
function startVerification(params: SessionVerifyCreateParams): Promise
```
...
Initiates a verification flow using passkeys. Returns a [`SessionVerification`](/reference/javascript/types/session-verification) instance with its status and supported factors.

```typescript
function verifyWithPasskey(): Promise
```
