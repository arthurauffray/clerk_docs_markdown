...
> An interface that describes an API key used to authenticate requests to your application's API endpoints.
...
- **`secret?`** `string | undefined`
  ...
  The API key secret. **This property is only present in the response from [`create()`](/reference/javascript/api-keys#create) and cannot be retrieved later.**
