...
> A session for a logged-in user that is either fully active or waiting to complete additional verification steps.
...
It contains all properties from the [`Session`](/reference/javascript/session) object, but guarantees that the `user` property is non-null and the `status` is either `active` or `pending`.
...
- **`currentTask`** [`SessionTask`](/reference/javascript/types/session-task)
