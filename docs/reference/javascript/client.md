...
- **`lastAuthenticationStrategy`** <code>[LastAuthenticationStrategy](/reference/javascript/types/last-authentication-strategy) | null</code>

  The last authentication strategy used by the client, or `null` if there is none.

    ---

- **`sessions`** [`Session[]`][session-ref]

  A list of sessions that have been created on this client.

    ---

- **`signedInSessions`** [`SignedInSessionResource[]`][signed-in-session-ref]
...
- **`signIn`** <code>[SignIn](/reference/javascript/sign-in) | null</code>

  The current sign in attempt, or `null` if there is none.

    ---

- **`signUp`** <code>[SignUp](/reference/javascript/sign-up) | null</code>
...
```typescript
function removeSessions(): Promise
```
