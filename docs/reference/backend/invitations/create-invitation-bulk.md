...
> Use Clerk's JS Backend SDK to create multiple invitations for the given email addresses, and send the invitation emails.

Creates multiple [`Invitation`](/reference/backend/types/backend-invitation)s in bulk for the given email addresses and sends the invitation emails.
...
> [!CAUTION]
>
> This endpoint is [rate limited](/guides/how-clerk-works/system-limits#backend-api-requests) to **25 requests per hour** per application instance.

```ts
function createInvitationBulk(params: CreateBulkParams): Promise
```

## Parameters

- **`params`** [`CreateBulkParams[]`](#create-bulk-params)
...
This method in the SDK is a wrapper around the BAPI endpoint `POST/invitations/bulk`. See the [BAPI reference](/reference/backend-api/tag/invitations/post/invitations/bulk) for more information.
