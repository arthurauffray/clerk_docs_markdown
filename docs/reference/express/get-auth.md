...
The `getAuth()` helper retrieves authentication state from the `request` object. See the [Next.js reference documentation](/reference/nextjs/pages-router/get-auth) for more examples on how to use the returned `auth` object.

## Usage

The `getAuth()` helper can be used to protect routes based on authentication status, authorization status, and token type. It also offers more granular control over how to handle unauthenticated users - you can redirect them to the sign-in page, return a `401` status code, or perform whatever action you need. Unlike [`requireAuth()`](/reference/express/require-auth), it **can** be used to protect API routes.

> [!QUIZ]
> When should you use `getAuth()` instead of [`requireAuth()`](/reference/express/require-auth)?
>
> ---
>
...
For more examples on how to use `getAuth()` to perform authorization checks, see the [dedicated guide](/guides/secure/authorization-checks).
