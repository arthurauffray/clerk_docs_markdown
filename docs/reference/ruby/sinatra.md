...
> The Clerk Ruby SDK provides a Sinatra extension to integrate Clerk into your Sinatra app.
...
For more information, see [Faraday's documentation](https://lostisland.github.io/faraday/#/).
  ...
  The `clerk` object provides access to the [Ruby SDK's available methods](/reference/ruby/overview#available-methods). To get access to the `clerk` object, you must `register` the `Sinatra::Clerk` extension.
...
For actions requiring additional security, Clerk provides a `reverify` filter that prompts users to re-authenticate. This filter accepts an optional [preset](/reference/ruby/overview#reverification) parameter to customize the reverification requirements.
