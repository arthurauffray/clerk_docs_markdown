...
> The Clerk Ruby SDK provides a Rack middleware and a gem to integrate Clerk into your Rails app.
...
For actions requiring additional security, Clerk provides a `:require_reverification!` filter that prompts users to re-authenticate. This filter accepts an optional [preset](/reference/ruby/overview#reverification) parameter to customize the reverification requirements.
