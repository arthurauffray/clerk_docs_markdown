...
> Clerk's useLocalCredentials() hook enables you to store and access a user's password credentials on their device.
...
> Be aware that this works only for sign-in attempts using a [password](/guides/configure/auth-strategies/sign-up-sign-in-options#password).
...
- **`setCredentials()`** <code>(params: [LocalCredentials](#local-credentials)) => Promise\<void></code>
...
To learn how to use the `useLocalCredentials()` hook, see the [reference doc](/guides/development/local-credentials).
