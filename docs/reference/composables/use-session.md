...
> Access and manage the current user's session in your Vue application with Clerk's useSession() composable.

The `useSession()` composable provides access to the current user's [`Session`](/reference/javascript/session) object, as well as helpers for setting the active session.
...
- **`session`** <code>Ref\<[Session](/reference/javascript/session)></code>
